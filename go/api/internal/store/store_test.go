...
// TestListEntries_RepostAndStarCounts verifies that ListEntries populates
// RepostCount (distinct users who shared the article URL) and StarCount
// (distinct users who starred it) from the local shared_articles and
// entry_stars tables, so the web client can render them without a second
// fetch per entry.
func TestListEntries_RepostAndStarCounts(t *testing.T) {
	s := testDB(t)
	ctx := context.Background()

	viewer := seedUser(t, s, "counts-viewer@example.com")
	feedID := seedFeed(t, s, viewer, nil, "Counts Feed")
	other1 := seedUser(t, s, "counts-other1@example.com")
	other2 := seedUser(t, s, "counts-other2@example.com")

	const title = "Counts Entry"
	entryID := seedEntry(t, s, viewer, feedID, title, "unread", false)
	articleURL := fmt.Sprintf("https://example.com/%s", title)

	// Two distinct users share the article -> repost count 2.
	if _, err := s.DB.Exec(
		`INSERT INTO shared_articles (user_id, article_url, entry_id, title) VALUES ($1,$2,$3,$4)`,
		other1, articleURL, entryID, title,
	); err != nil {
		t.Fatalf("seed share 1: %v", err)
	}
	if _, err := s.DB.Exec(
		`INSERT INTO shared_articles (user_id, article_url, entry_id, title) VALUES ($1,$2,$3,$4)`,
		other2, articleURL, entryID, title,
	); err != nil {
		t.Fatalf("seed share 2: %v", err)
	}
	// Two distinct users star the article -> star count 2 (viewer + other1).
	if _, err := s.DB.Exec(
		`INSERT INTO entry_stars (user_id, article_url, entry_id) VALUES ($1,$2,$3)`,
		viewer, articleURL, entryID,
	); err != nil {
		t.Fatalf("seed star 1: %v", err)
	}
	if _, err := s.DB.Exec(
		`INSERT INTO entry_stars (user_id, article_url, entry_id) VALUES ($1,$2,$3)`,
		other1, articleURL, entryID,
	); err != nil {
		t.Fatalf("seed star 2: %v", err)
	}

	entries, err := s.ListEntries(ctx, viewer, &feedID, nil, "", nil, "", "", 50, 0)
	if err != nil {
		t.Fatalf("ListEntries failed: %v", err)
	}
	var got *Entry
	...
	if got == nil {
		t.Fatalf("entry %d not in ListEntries result", entryID)
	}
	if got.RepostCount != 2 {
		t.Errorf("RepostCount = %d, want 2", got.RepostCount)
	}
	if got.StarCount != 2 {
		t.Errorf("StarCount = %d, want 2", got.StarCount)
	}
}
