...
// formatAPIError renders an API error response as "(status): detail", the
// same text printError writes to stderr.
func formatAPIError(status int, errModel *sunred.ErrorModel) string {
	if errModel != nil && errModel.Detail != nil {
		return fmt.Sprintf("(%d): %s", status, *errModel.Detail)
	}
	if errModel != nil && errModel.Title != nil {
		return fmt.Sprintf("(%d): %s", status, *errModel.Title)
	}
...
func printError(status int, errModel *sunred.ErrorModel) {
	fmt.Fprintf(os.Stderr, "error %s\n", formatAPIError(status, errModel))
}
