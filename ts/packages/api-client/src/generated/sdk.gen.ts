 ...
 *
 * Fetches and parses all of the user's subscribed feeds that are due for refresh, inserting any new entries. Use this to get the latest articles without waiting for the scheduler.
 */
export const refreshAllFeeds = <ThrowOnError extends boolean = false>(options?: Options<RefreshAllFeedsData, ThrowOnError>): RequestResult<RefreshAllFeedsResponses, RefreshAllFeedsErrors, ThrowOnError> => (options?.client ?? client).post<RefreshAllFeedsResponses, RefreshAllFeedsErrors, ThrowOnError>({ url: '/v1/feeds/refresh-all', ...options });
