...
// hash of the article URL, matching the feed processor's hash so the
// (feed_id, hash) unique constraint prevents a duplicate row when the sharer
// subscribes to the same source feed. When a row already exists (conflict),
// its id is looked up and reused so the share links to the existing entry.
// Returns the entry id (0 if the share carries no feed_url or on insert
// failure).
